...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    szTime = SysTime.Date
    InsBufLine(hbuf, ln + 4,  " * @@date			@szTime@")
    InsBufLine(hbuf, ln + 5,  " * @@version			³õ¸å")
    InsBufLine(hbuf, ln + 6,  " * @@par				°æÈ¨ËùÓÐ (C), 2013-2023, ÌìµÀ³êÇÚ")
    InsBufLine(hbuf, ln + 7, " * @@par History:")
    InsBufLine(hbuf, ln + 8, " * 1.ÈÕ    ÆÚ: 		@szTime@")

    if( strlen(szMyName)>0 )
    {
       InsBufLine(hbuf, ln + 9, " *   ×÷    Õß: 		@szName@")
    }
    else
    {
       InsBufLine(hbuf, ln + 9, " *  ×÷    Õß:		 #")
    }
    InsBufLine(hbuf, ln + 10, " *   ÐÞ¸ÄÄÚÈÝ:	´´½¨ÎÄ¼þ")    
    InsBufLine(hbuf, ln + 11, "*/")
    if(strlen(szContent) != 0)
    {
        return
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    {       
            ln = ln + 1
            InsBufLine(hbuf, ln+2, " *@@param[in]		ÎÞ")
    }
    InsBufLine(hbuf, ln+3, " *@@param[out]	ÎÞ")
